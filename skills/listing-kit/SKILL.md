...
Run `scripts/validate/validate-listing.sh` (pass the app root) — it checks every asset and metadata field against `references/stores/*.md`: character-limit overflows, screenshot dimensions vs. recognized device sizes, format (RGB/no-alpha/24-bit), Play's ≤2:1 aspect + 8MB, required assets (≥1 iPhone size; iPad screenshots when the app supports iPad — detected across Expo/native/Flutter; Play feature graphic + icon), and runs the secret scan. It validates **only the store(s) actually present**, so a single-store listing isn't failed for the store it never targeted. It exits non-zero on any failure. If a previous screenshot set exists (e.g. a prior commit or a backup dir), also run `scripts/validate/visual-diff.sh` (`<previous-dir> <current-dir>`) to get a per-screen regression report (added/removed/changed); it's informational and never blocks.
